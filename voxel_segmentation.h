...
//Type definition
typedef  pcl::PointCloud<pcl::PointXYZRGBA>::Ptr PTXYZRGBAPtr;
typedef  pcl::PointCloud<pcl::PointXYZRGBA> PTXYZRGBA;
typedef  pcl::PointCloud<pcl::PointXYZRGB>::Ptr PTXYZRGBPtr;
typedef  pcl::PointCloud<pcl::PointXYZRGB> PTXYZRGB;
typedef  pcl::PointCloud<pcl::PointXYZ>::Ptr PTXYZPtr;
typedef  pcl::PointCloud<pcl::PointXYZ> PTXYZ;
 ...
 namespace pcl
 {
   ...
   class  VoxelBasedSegmentation: public pcl::octree::OctreePointCloud<PointT>
   {
	...
	public:
		...
		//Member functions
		int
		getCloudPointNum(PTXYZPtr input_data)
		{
		...
		setVoxelCenters()
		{
			...
			PTXYZPtr temp_voxel_centers_cloud (new PTXYZ);
		...
		void
		findAllVoxelRegional(float region_size)
		{
			//Build kd-tree of voxels' centers
			this->buildVoxelCentersKdtree();
			this->region_resolution_=region_size;

			//Search radius
			double search_radius;
			search_radius=region_size;
			...
			for (int i=0; i<centers_num_;i++)
			{
				...
				{
					//Add elements in the vector
					voxels_regional_idx_.push_back(vector<int>(pointIdxRadiusSearch.size()+1,0));
					voxels_regional_idx_[i][0]=pointIdxRadiusSearch.size();//The first element records the num of adjacencies
					...
					{
						//Records
						voxels_regional_idx_[i][j]=pointIdxRadiusSearch[j-1];
						//std::cout	<< "    "  << voxels_adjacency_idx_[i][j]<< std::endl;
					}
				}
			}
		...
		std::vector<int>
		getOneVoxelRegional(int voxel_id)
		{
			////Test
			//std::cout	<< " "<< std::endl;
			//std::cout	<< " Find adjacent voxels of voxel  "<< voxel_id <<std::endl;
			//std::cout	<< " Node:   "<< voxel_id<< "  having  "<< voxels_adjacency_idx_[voxel_id][0] <<" adjacent voxels "<<std::endl;
			std::vector<int> region_idx;//=Eigen::VectorXi::Zero(voxels_adjacency_idx_[voxel_id][0]);
			
			//Traverse
			for (size_t j = 1; j < voxels_regional_idx_[voxel_id][0]+1; j++)//Find the idx of recorded adjacent voxels 
			{
				////Records
				//std::cout	<< "    "  << voxels_adjacency_idx_[voxel_id][j]<< std::endl;					
				region_idx.push_back(voxels_regional_idx_[voxel_id][j]);
				//std::cout	<< "    "  << adjacency_idx[j-1]<< std::endl;
			}
		...
		//Features
		void
		calcualteVoxelCloudAttributes(PTXYZPtr input_cloud)
		{
			//Iteration index
			VoxelBasedSegmentation<pcl::PointXYZ>::LeafNodeIterator iter_leaf (this); //Iteration index for all leaf nodes in the tree
			...
			while (*++iter_leaf)// The initial node is neglected!
			{
				...
				PTXYZPtr voxel_cloud (new PTXYZ);
		...
		//Segmentation
		void
		segmentVoxelCloudWithGraphModel(float cut_thred,float sig1, float sig2, float sig3, float sig4, float sig5, float sig6)      //Segmentation VGS
		{
			...
			for(int i=0;i<voxels_num_;i++)
			{
				...
				//Judgement
				if(voxel_used_[i])//Voxel used or not
				{
					...
					//Graph building
					voxel_adjgraph=this->buildAdjacencyGraph(voxel_adjidx,sig1,sig2,sig3,sig4,sig5,sig6);
			...
			//Closest checking
			this->closestCheck(sig1, sig2, sig3, sig4, sig5, sig6);
			//Voxel Clustering
		...
		drawColorMapofVoxels(pcl::PolygonMesh::Ptr output_mesh)//Draw the colored boxes of the voxels
		{
			...
			while (*++iter_leaf)
			{
				...
				PTXYZRGBPtr voxel_vertices (new PTXYZRGB);
    ...
    private:
		...
		float region_resolution_;		//Resolution(size) of the local region
		...
		PTXYZPtr points_cloud_;			//PT Ptr of all points
		PTXYZPtr voxel_centers_cloud_;  //PT Ptr of all center points of voxels

		//Member functions
		//Features
		std::vector<float>
		calculateEigenFeatures(PTXYZPtr input_cloud)//Eigen features
		{
			//Parameter setting
			...
			{
				eig_e1=(float)eig_values[0]/sqrt(pow(eig_values[0],2)+pow(eig_values[1],2)+pow(eig_values[2],2));
				eig_e2=(float)eig_values[1]/sqrt(pow(eig_values[0],2)+pow(eig_values[1],2)+pow(eig_values[2],2));
				eig_e3=(float)eig_values[2]/sqrt(pow(eig_values[0],2)+pow(eig_values[1],2)+pow(eig_values[2],2));
		...
		pcl::PointXYZ
		calculateVoxelCentroid(PTXYZPtr input_cloud)
		{
			//Parameter setting
		...
		pcl::Normal
		calculateVoxelNorms(PTXYZPtr input_cloud)
		{
			//Parameter setting
			Eigen::Vector3f eig_values;
			Eigen::Matrix3f eig_vectors;
		...
		void
		calculateWeightedCorvariance(PTXYZPtr input_cloud, Eigen::Matrix3f *output_cor)
		{
			//Settings 
		...
		void
		calculateCorvariance(PTXYZPtr input_cloud, Eigen::Matrix3f *output_cor)
		{
			//Settings 
			pcl::PointXYZ point_can;	// Candidate point
			Eigen::Vector3f key_coor=Eigen::Vector3f::Zero(3,1);	// Coordinates of key point 
		...
		measuringDistance(std::vector<float> v1_center, std::vector<float> v2_center, std::vector<float> v1_norm, std::vector<float> v2_norm, std::vector<float> v1_eigen, std::vector<float> v2_eigen)
		{
			...
			float dist_space=100;	//s
			float dist_norm=100;	//n:convex
			float dist_eigen=100;	//e
			float dist_stair=100;	//n:stair distance
			float dist_angle=100;	//n:norm angle
			...
			if(v1_center.size()>1 && v2_center.size()>1) //space: euclidean distance
			{
				...
				//dist_space=dist_v1_v2 / (this->graph_resolution_);
			...
			if(v1_norm.size()>1 && v2_norm.size()>1)   //norm: cos angle
			{
				...
				//dist_cos_dist1=abs(cos_v1_dist+cos_v2_dist); //For constrainting stair effect
				//dist_norm=(float)(pow(a_1_2,2))*10;
				//dist_norm=(float)(pow((a_1-a_2),2)+pow((PI-a_1-a_2),2)+pow(a_1_2,2))/(PI*PI);

				dist_norm=abs(a_1-a_2);
				if(a_d_s>thred_singular)
				{
					dist_angle=abs(a_1_2);	
				}
				dist_stair=abs(PI-a_1-a_2);

				//if((a_1-a_2)>(float)-1*PI/8)
				//{
				//	dist_norm=(float)abs(a_1_2)/(PI);
				//}
				//else
				//{
				//	dist_norm=10;
			...
			similarity_dist.push_back(dist_norm);	//N
		...
		float
		distanceProbability(std::vector<float> dist_all, float sig_a, float sig_n, float sig_t, float sig_s, float sig_e, float sig_weight)
		{
			//Parameters and settings
			float dist_a, dist_n, dist_t, dist_s, dist_e;
			float similarity_prob=0, prob_a=0, prob_n=0, prob_t=0, prob_s=0, prob_e=0;

			dist_a=dist_all[0]; //angle
			dist_n=dist_all[1]; //convex
			dist_t=dist_all[2]; //stair
			dist_s=dist_all[3]; //space
			dist_e=dist_all[4]; //eigen

			//Probability calculation
			prob_a=exp(-0.5*(float)pow(dist_a,2)/sig_a);
			prob_n=exp(-0.5*(float)pow(dist_n,2)/sig_n);
			prob_t=exp(-0.5*(float)pow(dist_t,2)/sig_t);			
			prob_s=exp(-0.5*(float)pow(dist_s,2)/sig_s);
			prob_e=exp(-0.5*(float)pow(dist_e,2)/sig_e);

			//Test
			//std::cout<<prob_a<<"  "<<prob_n<<"  "<<prob_t<<"  "<<prob_s<<"  "<<prob_e<<std::endl;

			similarity_prob=prob_a*prob_n*prob_t*prob_s*prob_e;
		...
		Eigen::MatrixXf
		buildAdjacencyGraph(std::vector<int> voxels_idx, float sig1, float sig2, float sig3, float sig4, float sig5, float sig6)		//Building simple adjacency Graph: static graph only connectivity considered
		{
			//Parameters and settings
			...
			//voxels_idx=this->getOneVoxelAdjacency(voxel_idx);
			...
			{
				//std::vector<float> voxel1_position, voxel1_features;
				voxel1_position.clear();
				voxel1_normal.clear();
				voxel1_eigen.clear();
				temp_center=voxel_centroids_[voxels_idx[i]];//temp_center=voxel_centers_[voxels_idx[i]];
				...
				{
					//std::vector<float> voxel2_position, voxel2_features;
					voxel2_position.clear();
					voxel2_normal.clear();
					voxel2_eigen.clear();
					temp_center=voxel_centroids_[voxels_idx[j]];//temp_center=voxel_centers_[voxels_idx[j]];
					...
					if(i!=j)
					{
						...
						//similarity_voxels=distanceWeight(dist_all, sig2, sig4, sig5, sig6);
						similarity_voxels=distanceProbability(dist_all, sig1, sig2, sig3, sig4, sig5, sig6);
		...
		cutGraphSegmentation(float thrd_value, Eigen::MatrixXf graph_matrix, std::vector<int> voxel_adjidx)       //Simple test, cut the graph with a given threshold
		{
			...
			for (int i = 0; i < weight_size; i++) 
			{
				...
				v1_idx=floor(double(sorted_idx[i]/graph_size));
				v2_idx=sorted_idx[i]-v1_idx*graph_size;
				w_temp=weight_array[i];
		...
		clusteringVoxels()		//Clustering of voxels		
		{
			...
			//Clustering
			for(int i=0;i<voxels_num_;i++)// loop + recrusion
			{
				//Test
				//std::cout<<"voxel: "<<i<<" connecting  "<<voxels_connect_idx_[i].size()<<std::endl; 

				if(!voxel_clustered_[i])
				{	
					//std::cout<<i<<std::endl;

					std::vector<int> *inthis_cluster_idx=new std::vector<int>;

					//inthis_cluster_idx->push_back(i); //Remove the points in unclustered voxels 

					voxel_clustered_.at(i)=true;

					this->recursionSearch(voxels_connect_idx_[i],inthis_cluster_idx);

					inthis_cluster_idx->push_back(i); //Don't forget to 
					
					clusters_voxel_idx_.push_back(*inthis_cluster_idx);
				}
			}
		...
		void
		closestCheck(float sig1, float sig2, float sig3, float sig4, float sig5, float sig6)
		{
			//Settings
					...
					if(voxels_adjacency_idx_[i].size()>this->voxel_adjacency_min_)// But if this "single" voxel has only few adjacencies, we will give it up.
					{
						...
						for (int j=0;j<voxels_adjacency_idx_[i].size();j++)
						{
							if(voxels_connect_idx_[voxels_adjacency_idx_[i][j]].size()>1) // We cannot merge a single voxel to another single voxel
							{
								...
								//voxel2_features=voxel_features_[voxels_adjacency_idx_[i][j]];
								//voxel2_colors=voxel_colors_[voxels_adjacency_idx_[i][j]];

								dist_vector=measuringDistance(voxel1_position, voxel2_position, voxel1_normal, voxel2_normal, voxel1_eigen, voxel2_eigen);		
								temp_dis=distanceProbability(dist_vector, sig1, sig2, sig3, sig4, sig5, sig6);
